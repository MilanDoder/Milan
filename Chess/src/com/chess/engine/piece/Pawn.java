...
public class Pawn extends Piece {
	...
	public Collection<Move> calculateIlegalMove(Board board) {
		...
		for(final int currentCoordinateOffset: CANDIDATE_MOVE_COORDINATE){
			...
			final int candidateDestinationCoordinate = this.piecePositions +(this.getPieceAlliance().getDirection()* currentCoordinateOffset);
			...
			if(currentCoordinateOffset==8 && !board.getTile(candidateDestinationCoordinate).isTileOccupied()){
				legalMove.add(new MajorMove(board,this, candidateDestinationCoordinate));
					...
					(BoardUtils.SECOND_ROW[this.piecePositions] && this.getPieceAlliance().isBlack() ) 
					|| (BoardUtils.SEVEN_ROW[this.piecePositions])&& this.getPieceAlliance().isWhite()){
				...
				if(!board.getTile(behindCandidanteDestinastionCoordinate).isTileOccupied() 
					...
					!((BoardUtils.EIGHT_COLUMN[this.piecePositions] && this.pieceAlliance.isWhite()) ||
					(BoardUtils.FIRST_COLUMN[this.piecePositions] && this.pieceAlliance.isBlack()))){
